...
class CaptioningTransformer(nn.Module):
    ...
    def __init__(self, word_to_idx, input_dim, wordvec_dim, num_heads=4,
        ...
        # Weights for processing of features (input), before transformer.
    ...
    def forward(self, features, captions):
        ...
        # W = wordvec_dim, both features and captions must have the same wordvec_dim.

        # Apply linear transformation of input features. (N,D) -> (N,W) -> (N,1,W)
        feat_proj = self.visual_projection(features).unsqueeze(1)
        print('feat_proj shape: ', feat_proj.shape)

        # Change captions into its word embedded form. (N,T) -> (N,T,W)
        captions_embed = self.embedding(captions)
        print('captions_embed shape: ', captions_embed.shape)

        # Add positional encoding to word embedded captions. (N,T,W) -> (N,T,W)
        captions_embed = self.positional_encoding(captions_embed)
        print('captions_embed shape: ', captions_embed.shape)

        # Add mask, used to make decoder mask out the future timestpes in captions.
        # Must be shape of (T,T), since aligmnet scores last dimensions is (S,T)
        # or (T,T).
        tgt_mask = torch.tril(torch.ones(T,T, device=captions_embed.device, dtype=captions_embed.dtype))

        # Simply apply the Transformer decoder on the embedded captions, making sure it 
        # attends to the image's features. scores = (N,T,W)
        scores = self.transformer(tgt=captions_embed, memory=feat_proj, tgt_mask=tgt_mask)
        print('scores shape: ', scores.shape)

        # Once scores is computed, apply the last linear projection to the scores.
        # This is the last FC layer, after the decoder block.
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(self, tgt, memory, tgt_mask=None):
        ...
        """
        Masked self-attention, where the input for the Q,K,V is the captions embedded.
        Apply dropout, residual connection, the layer norm.
        """
        ...
        # encoder layer.
        """
        No masking for this self-attention, Q is the captions embedded, K,V is the 
        features. This makes the captions attend to the features. 
        """
        ...
        # Pass
        # Final computation before the end of the decoder block.
