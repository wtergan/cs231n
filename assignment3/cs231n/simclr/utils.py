...
def train(model, data_loader, train_optimizer, epoch, epochs, batch_size=32, temperature=0.5, device='cuda'):
    ...
    for data_pair in train_bar:
        ...
        # Do the model twice, one for x_i and once for x_j, both of which representing 
        # the positive pair for the original input image. The model returns the 
        # normalized features, as well as the normalized output. We only need the output.
        (_, out_left) = model(x_i)
        (_, out_right) = model(x_j)

        # Then, we can compute the contrastive loss for both out_left and out_right.
        # we will use the vectorized form for computation.
