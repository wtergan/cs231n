...
def svm_loss_naive(W, X, y, reg):
    ...
    for i in range(num_train):
        ...
        for j in range(num_classes):
            ...
            if margin > 0:
                loss += margin
                #gradient of loss wrt predicted class j
                dW[:, j] += X[i]
                #gradient of loss wrt true label class y[i]
                dW[:, y[i]] -= X[i]
...
def svm_loss_vectorized(W, X, y, reg):
    ...
    margin[margin > 0] = 1.0
    margin[margin < 0] = 0.0
    margin[range(num_train), y] = -1.0 * np.sum(margin, axis=1)
    dW = (np.dot(X.T, margin) / num_train) + W * reg
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
